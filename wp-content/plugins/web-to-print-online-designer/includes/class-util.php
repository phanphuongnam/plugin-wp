            ...
            if( isset( $response['response'] ) && isset( $response['response']['code'] ) && $response['response']['code'] == '200' ){
                ...
                if( isset( $res_arr[1] ) ){
                    for( $i = 1; $i <= $res_arr[1]; $i++ ){
                        ...
                        $cloud_link = 'https://pdf2image-api.s3.amazonaws.com/' . $res_arr[0] . '/page_' . $i . '.' . $format;
...
function nbd_sort_file_by_side_callback( $file1, $file2 ){
    ...
    if( preg_match_all('/\d+/', $base_name1, $matches) ){
        if( count( $matches[0] ) ){
            $index1 = absint( $matches[0][0] );
        }
    }

    if( preg_match_all('/\d+/', $base_name2, $matches) ){
        if( count( $matches[0] ) ){
            $index2 = absint( $matches[0][0] );
        }
    }
